...
public class CommunityModifyController extends HttpServlet {
    ...
    private void daoModify(HttpServletRequest req, HttpServletResponse resp, CommunityDTO dto)
            throws IOException, ServletException {
        CommunityDAO dao = new CommunityDAO();
        int postid = dao.CommunityModify(dto);
        if (postid > 0) {
            resp.sendRedirect("/hackthebox/community/posts?id=" + postid);
        } else {
            try {
